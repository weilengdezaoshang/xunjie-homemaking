...
Page({
    ...
    handleUploadDelete (event) {
        const deleIndex = this.data.illustration.findIndex(item => item.key === event.detail.item.key);
        this.data.illustration.slice(deleIndex, 1);
    },
    handleSubmit () {
        this.selectComponent('form').validate(async (valid) => {
            if (!valid) {
            ...
            await Rating.createRating(this.data.order.id, this.data.formData.score,this.data.formData.content, illustration);
            wx.hideLoading();
